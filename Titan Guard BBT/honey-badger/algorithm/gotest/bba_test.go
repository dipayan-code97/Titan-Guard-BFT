...
func TestBBAStepByStep(t *testing.T) {
	...
	assert.True(t, bba.receivedBinaryValue[0]) // we are id (0)
	msgs := bba.Messages()
	assert.Equal(t, 1, len(msgs))
	assert.IsType(t, &BinaryValueRequest{}, msgs[0].Message)
	assert.True(t, msgs[0].Message.(*BvalRequest).Value)

	// Sent input from node 1
	bba.handleBvalRequest(uint64(1), true)
	assert.True(t, bba.receivedBinaryValue[1])
	...
	assert.True(t, bba.receivedBinaryValue[2])
	msg := bba.Messages()
	assert.Equal(t, 1, len(msg))
	assert.IsType(t, &AuxillaryRequest{}, msg[0].Message)
	assert.True(t, msg[0].Message.(*AuxillaryRequest).Value)
	assert.True(t, bba.receivedAuxillary[0]) // our id

	// Let's assume node 1 and node 2 are good nodes and also sent their AUX
	// message
	bba.handleAuxRequest(uint64(1), true)
	assert.True(t, bba.receivedAuxillary[1])
...
func TestNewBBA(t *testing.T) {
	...
	assert.Equal(t, 0, len(bba.receivedBinaryValue))
	assert.Equal(t, 0, len(bba.receivedAuxillary))
...
func TestAdvanceEpochInBBA(t *testing.T) {
	...
	bba.receivedAuxillary = map[uint64]bool{
		1:    false,
		3949: true,
	}
	bba.advanceEpoch()
...
func makeBBAInstances(n int) []*BinaryByzantineAgreement {
	bbas := make([]*BinaryByzantineAgreement, n)
	for i := 0; i < n; i++ {
