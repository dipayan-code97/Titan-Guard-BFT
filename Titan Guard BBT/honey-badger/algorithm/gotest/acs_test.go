...
func testCommonSubset(t *testing.T, inputs map[int][]byte) {
	...
	go func() {
		for {
			...
			case msg := <-messages:
				...
				for _, msg := range acs.messageQueue.messages() {
					go func(msg MessageTuple) {
						messages <- testMsg{acs.ID, msg}
					}(msg)
				}
				if output := acs.getACSInstance(); output != nil {
	...
	for nodeID, value := range inputs {
		assert.Nil(t, nodes[nodeID].setInputValue(value))
		for _, msg := range nodes[nodeID].messageQueue.messages() {
...
func TestACSOutputIsNilAfterConsuming(t *testing.T) {
	acs := BuildNewACS(Config{N: 4})
	output := map[uint64][]byte{
		1: []byte("this is it"),
	}
	acs.output = output
	assert.Equal(t, output, acs.getACSInstance())
	assert.Nil(t, acs.getACSInstance())
}
...
func makeACSNetwork(n int) []*ACS {
	network := make([]*ACS, n)
	for i := 0; i < n; i++ {
		network[i] = BuildNewACS(Config{N: n, ID: uint64(i), Nodes: makeids(n)})
		go network[i].run()
	}
	return network
}
